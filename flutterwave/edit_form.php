...
class enrol_flutterwave_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'tcost', get_string('tcost', 'enrol_flutterwave'), array('size' => 4));
        $mform->setType('tcost', PARAM_RAW);
        $mform->setDefault('tcost', format_float($plugin->get_config('tcost'), 2, true));
        $mform->addHelpButton('tcost', 'tcost', 'enrol_flutterwave');
    ...
    public function validation($data, $files) {
        ...
        $tcost = str_replace(get_string('decsep', 'langconfig'), '.', $data['tcost']);
        if (!is_numeric($tcost)) {
            $errors['tcost'] = get_string('total_costerror', 'enrol_flutterwave');
        }
